...
def dict2model(df, idcol, cols, save_name='feedid_raw_manu_tag_tfidf'):
    ...
    # m.vocab = emb_dic # old version
    m.key_to_index = emb_dic
    m.vectors = np.array(list(emb_dic.values()))
    # my_save_word2vec_format(binary=True, fname='train.bin', total_vec=len(emb_dic), vocab=m.vocab, vectors=m.vectors) # old version
    my_save_word2vec_format(binary=True, fname='train.bin', total_vec=len(emb_dic), vocab=m.key_to_index,
                            vectors=m.vectors)
