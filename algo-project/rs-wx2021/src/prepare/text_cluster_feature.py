...
if find_best_K:
    ...
    df = pd.read_pickle(f'{REFORM_DATA_PATH}/feed_author_text_features_fillna_by_author_rm0.pkl')
    ...
    author_machine_tag_emb = author_emb[~author_emb['author_machine_tag_tfidf_0'].isnull()][
        author_machine_tag_tfidf_cols]
...
df_clusters = df.merge(feed_cls_res, on='feedid', how='left').merge(author_cls_res, on='authorid', how='left')
...
reduce_mem_usage(df_clusters).to_pickle(f'{REFORM_DATA_PATH}/feed_author_text_features_fillna_by_author_clusters.pkl')
