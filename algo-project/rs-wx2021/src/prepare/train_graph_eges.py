...
def create_embedding_matrix(sparse_columns, varlen_sparse_columns, embed_dim, init_std=0.0001, padding=True,
                            device='cpu', mode='mean'):
    # sparse_columns => dict{'name':vocab_size}
    # Return nn.ModuleDict: for sparse features, {embedding_name: nn.Embedding}
    padding_idx = 0 if padding else None
    sparse_embedding_dict = {feat: nn.Embedding(sparse_columns[feat], embed_dim, padding_idx=padding_idx) for feat in
                             sparse_columns}

    if varlen_sparse_columns:
        varlen_sparse_embedding_dict = {
            feat: nn.EmbeddingBag(varlen_sparse_columns[feat], embed_dim, padding_idx=padding_idx, mode=mode) for feat
            in varlen_sparse_columns}
...
class EGES(nn.Module):
    def __init__(self, sparse_dict, varlen_sparse_dict=None, target_col='sku_id',
        ...
        self.input_embedding_dict = create_embedding_matrix(sparse_dict, varlen_sparse_dict, n_embed, init_std=0.0001,
                                                            padding=padding, device=device, mode='mean')
        self.out_embed = nn.Embedding(sparse_dict[target_col], n_embed, padding_idx=0 if padding else None)
        self.attn_embed = nn.Embedding(sparse_dict[target_col], k_side + 1, padding_idx=0 if padding else None)
...
class TextData(Dataset):
    def __init__(self, df, sparse_columns=['feedid', 'label', 'authorid', 'feed_machine_tag_tfidf_cls_32',
                                           'feed_machine_kw_tfidf_cls_17'], varlen_sparse_columns=[], device='cpu'):
        self.sparse_columns = sparse_columns
        self.varlen_sparse_columns = varlen_sparse_columns
        self.device = device
        self.data = {col: df[col].values for col in sparse_columns}
        if varlen_sparse_columns:
...
def train():
    ...
    textloader = DataLoader(textdata, batch_size=10000, shuffle=True, num_workers=10, drop_last=False, pin_memory=True)

    embedding_dim = 64
    model = EGES(vocab_dict, n_embed=embedding_dim, k_side=3, target_col='feedid', noise_dist=noise_dist, device=device,
