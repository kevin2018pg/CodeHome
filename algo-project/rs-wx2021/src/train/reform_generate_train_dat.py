...
# @Time    : 2022/4/5 15:35
# @Author  : west
# @File    : reform_generate_train_dat.py
# @Version : python 3.6
# @Desc    : 生成训练验证数据集
...
def process_pipe(feed_path, user_act_path, used_columns, used_sparse_cols, used_dense_cols, emb_dim=16,
    ...
    data = reform_preprocess_util.preprocess(feed_path, user_act_path)
    data_ds = reform_preprocess_util.down_sample(data, used_columns, sample_method=None, neg2pos_ratio=300,
                                                 user_samp='random', by_date=None, is_training=is_training)
    if (list(data_ds.head(2)['date_'])[0] == 15):  # test_data
        X_dic, y_arr, linear_feats, dnn_feats, lbe_dict = reform_preprocess_util.process_features(data_ds,
                                                                                                  used_sparse_cols,
                                                                                                  used_dense_cols,
                                                                                                  actions=ACTIONS,
                                                                                                  emb_dim=emb_dim,
        ...
        X_dic_train, y_arr_train, linear_feats, dnn_feats, lbe_dict = reform_preprocess_util.process_features(
            train_data,
            used_sparse_cols,
            used_dense_cols,
            actions=ACTIONS,
            ...
            behavior_feature_list=[
                'feedid',
                'authorid'], )
        X_dic_val, y_arr_val, linear_feats, dnn_feats, lbe_dict = reform_preprocess_util.process_features(val_data,
                                                                                                          used_sparse_cols,
                                                                                                          used_dense_cols,
                                                                                                          actions=ACTIONS,
                                                                                                          emb_dim=emb_dim,
