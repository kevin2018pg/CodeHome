...
# 初始化embedding矩阵并加载预训练embedding
def create_embedding_from_pretrained(feature_columns, init_std=0.0001, linear=False, sparse=False, device='cpu',
                                     pretrained_embedding_dict=None, frozen_pretrained=False):
    # Return nn.ModuleDict: for sparse features, {embedding_name: nn.Embedding}
    # for varlen sparse features, {embedding_name: nn.EmbeddingBag}
    sparse_feature_columns = list(filter(lambda x: isinstance(x, SparseFeat), feature_columns)) if len(
        feature_columns) else []
    ...
    #     embedding_dict[feat.embedding_name] = nn.EmbeddingBag(feat.dimension, embedding_size,sparse=sparse,mode=feat.combiner)
...
class MyBaseModel(BaseModel):
    def __init__(self, linear_feature_columns, dnn_feature_columns, l2_reg_linear=1e-5, l2_reg_embedding=1e-5,
                 init_std=0.0001, seed=1024, task='binary', device='cpu', gpus=None, pretrained_embedding_dict=None,
                 frozen_pretrained=None, num_tasks=4):
        ...
        #                                                        device=device,
        #                                                        pretrained_embedding_dict=pretrained_embedding_dict,
    ...
    def fit(self, train_loader=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
            validation_data=None, val_userid_list=None, shuffle=True, callbacks=None, num_warm_epochs=1,
            lr_scheduler=True, scheduler_epochs=5, reduction='sum', task_weight=None, task_dict=None, num_workers=2,
            scheduler_method='cos', early_stop_uauc=0.55, label_smoothing=0.2):
        """
        :param train_loader: x->Numpy array of training data (if the model has a single input), or list of Numpy arrays (if the model has multiple inputs).If input layers in the model are named, you can also pass a
            dictionary mapping input names to Numpy arrays. y->Numpy array of target (label) data (if the model has a single output), or list of Numpy arrays (if the model has multiple outputs).
        ...
        :param val_userid_list:
        :param shuffle: Boolean. Whether to shuffle the order of the batches at the beginning of each epoch.
        :param callbacks: List of `deepctr_torch.callbacks.Callback` instances. List of callbacks to apply during training and validation (if ). See [callbacks](https://tensorflow.google.cn/api_docs/python/tf/keras/callbacks). Now available: `EarlyStopping` , `ModelCheckpoint`
        :param num_warm_epochs:
        :param lr_scheduler: 学习率更新
        :param scheduler_epochs:
        :param reduction:
        :param task_weight:
        :param task_dict:
        :param num_workers:
        :param scheduler_method:
        :param early_stop_uauc:
            ...
            if scheduler_method == 'linear':
                scheduler = get_linear_schedule_with_warmup(optim,
                                                            ...
                                                            num_training_steps=int(
                                                                len(train_loader)) * scheduler_epochs)
            else:
                ...
                for name, result in train_result.items():
                    ...
                    if do_validation:
                        for name in self.metrics:
                            for i in range(self.num_tasks):
                                task_name = task_dict[i] + '_' + name
                                eval_str += " - " + "val_" + task_name + ": {0: .4f}".format(
                                    epoch_logs["val_" + task_name])
    ...
    def predict(self, test_loader):
        ...
        :param test_loader: The input data, as a Numpy array (or list of Numpy arrays if the model has multiple inputs).
    ...
    def compile(self, optimizer, learning_rate=0.01, loss=None, metrics=None):
        """
...
class MOE(MyBaseModel):
    def __init__(self, linear_feature_columns, dnn_feature_columns, use_fm=False, use_nfm=False,
                 dnn_hidden_units=(256, 128), l2_reg_linear=0.00001, l2_reg_embedding=0.00001, l2_reg_dnn=0,
                 init_std=0.0001, seed=1024, dnn_dropout=0, bi_dropout=0, dnn_activation='relu', dnn_use_bn=True,
                 task='binary', device='cpu', gpus=None, pretrained_embedding_dict=None, frozen_pretrained=False,
                 num_tasks=4, pretrained_user_emb_weight=None, pretrained_author_emb_weight=None,
        ...
        dnn_hidden_units = dnn_hidden_units if len(dnn_hidden_units) == num_tasks and isinstance(dnn_hidden_units[0],
                                                                                                 tuple) else [
            dnn_hidden_units for i in range(num_tasks)]
        ...
        if self.pretrained_user:
            for emb_w in pretrained_user_emb_weight:
                self.user_emb.append(PretrainedEmbedding(emb_w.shape[0], emb_w.shape[1], emb_w).to(device))
        ...
        if self.pretrained_author:
            for emb_w in pretrained_author_emb_weight:
                self.author_emb.append(PretrainedEmbedding(emb_w.shape[0], emb_w.shape[1], emb_w).to(device))
                self.pretrained_emb_dim += emb_w.shape[1]
            self.author_emb = nn.ModuleList(self.author_emb)
        if self.pretrained_feed:
            for emb_w in pretrained_feed_emb_weight:
                self.feed_emb.append(PretrainedEmbedding(emb_w.shape[0], emb_w.shape[1], emb_w).to(device))
                self.pretrained_emb_dim += emb_w.shape[1]
        ...
        self.dnn = nn.ModuleList([DNN(dnn_input_dim, dnn_hidden_units[i], activation=dnn_activation, l2_reg=l2_reg_dnn,
                                      dropout_rate=dnn_dropout, use_bn=dnn_use_bn, init_std=init_std, device=device) for
