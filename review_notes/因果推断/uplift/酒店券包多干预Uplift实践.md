...
## 十一、三类业务的四层落地：样本、特征、模型、评估

> 飞猪广告参考：[《因果推断在阿里飞猪广告算法中的实践》](https://zhuanlan.zhihu.com/p/410582804)。三类问题共享“估计不同 treatment 下的潜在结果”这一模型地基，但样本构造、目标定义和策略层不同。

### 11.1 总览

| 业务 | Treatment | Outcome | 样本核心难点 | 推荐模型 | 最终决策 |
|---|---|---|---|---|---|
| 广告是否投放 | 有广告/无广告 | 平台整体GMV、转化、营收 | 广告由策略选择，组间分布差异大 | Matching + EffectNet/TARNet | 广告准入、预算/出价 |
| 悬浮球是否展示 | 展示/不展示 | 未来7日搜索量 | 产品规则导致非随机展示 | RCT + XGB T-Learner/DR | 展示人群圈选 |
| 券/红包 | 不发券 + K档券 | 转化、GMV、毛利 | 多 treatment、自然转化补贴浪费 | S-Learner基线 → TARNet/DR | 选择唯一券档 |
| 成本与ROI控制 | 上述所有动作 | 增量收益与成本 | 局部最优不等于全局预算最优 | 响应模型 + LP/MIP/拉格朗日 | 最大化总增量价值 |
...
但广告竞价由复杂策略决定，观察数据偏差更强；悬浮球更容易做稳定随机实验。
...
- treatment 取系统分配动作，而不是用户是否点击广告/悬浮球；
- 用户级活动应固定分桶，避免同一用户今天 treatment、明天 control；
- 搜索广告可能存在流量间竞争和预算干扰，可按 Query/流量桶/时间做实验；
- 保留真实分流概率，用于不等比例实验的 IPW/Policy Value。

##### 非随机广告样本：Original Space Matching

飞猪广告无法对全部线上流量随机投放，于是采用后采样：

1. 样本单位为一次搜索请求/结果页；
2. 使用**广告插入前**自然搜索队列的 pCTR 分布构造：

```text
Vmatch=[top1_pCTR,top2_pCTR,...,topN_pCTR]
```

3. 按城市、Query类目、设备、时段、流量入口做精确分层；
4. 对 control（无广告）页面全量建立 FAISS 索引；
5. 每个 treatment（有广告）页面检索 Top-K 个最相似 control；
6. 使用 caliper 丢弃距离仍然过远的样本；
7. K个control等权或按距离加权，形成匹配后的 treatment-control 数据。
...
\sum_{i:T_i=1}
\left[
Y_i-\sum_{j\in N_K(i)}w_{ij}Y_j
\right]
$$

匹配必须检查：

- 匹配前后各特征 SMD，经验上要求 `|SMD|<0.1`；
- 匹配距离、丢弃率、control复用次数；
- treatment/control 的 Query、城市、时段和 pCTR 分布；
- 有效样本量及匹配支持域覆盖。

> Matching 只能平衡已观察变量，无法修复未记录的广告主预算、竞价规则和人工配置。优先级仍是随机实验 > 完整日志下的 DR/Propensity > 原空间匹配 > 直接比较原始组。
...
1. 尽量补做用户级随机实验；
2. 无法随机时，回溯产品规则中所有 confounders；
3. 训练 `e(x)=P(T=1|X)`，采用 IPW/DR；
4. 或在用户画像、历史搜索强度、Query结构上 matching；
5. 删除 propensity 过近 0/1、没有 overlap 的样本。

#### B. 特征层

所有特征必须来自 treatment 前。

广告投放：

- 用户画像、活跃度、消费力、历史广告响应；
- Query、行业、城市、设备、时段；
- Search Rank Queue：自然商品的 pCTR/pCVR、价位、相关性分布；
- 广告插入前自然结果页的 topN pCTR 向量与统计量；
- 广告投放资格、竞价环境、广告库存、预算/pacing规则；
- 广告候选与自然Top商品的 pCTR/pCVR差值。

悬浮球：

- 历史1/3/7/30日搜索次数、活跃天数；
- 搜索类目、Query多样性和时段分布；
- 用户生命周期、会员等级、设备、渠道；
- 历史活动入口曝光和疲劳度；
- treatment 前页面状态。
...
这属于 S-Learner。文章中普通预估模型 AUC 尚可，但 Qini 为负；增加 DNN 后 AUC 提升，Uplift 指标反而更差，说明：

> Outcome预测准确，不代表 treatment effect 估计准确。

##### 改进1：Direct Method with Shortcut
...
\hat y=
f_{base}(h)
+T\cdot f_{effect}(h,e_T)
$$

其中 treatment embedding/effect branch 通过 shortcut 接到最后一层，避免深层非线性网络忽略单个 treatment 变量。

##### 改进2：Domain Adaptation / Multi-task

```text
Shared Representation
   ├─ Search/Control Head → μ₀(x)
   └─ AD/Treatment Head   → μ₁(x)
```
...
训练只监督实际 treatment head；推理同时计算：

$$
\hat\tau(x)=\sigma(z_1)-\sigma(z_0)
$$

这就是 TARNet 式双头模型。飞猪实践表明，多任务模型在匹配样本上有效，但在原始有偏样本上更容易过拟合分布差异，因此**模型结构不能替代样本去偏**。
...
$$
\hat\mu_1(x)=\hat\mu_0(x)+\hat\tau(x)
$$

$$
\hat y=(1-T)\hat\mu_0(x)
+T[\hat\mu_0(x)+\hat\tau(x)]
$$

Effect-Net 用结构强先验强迫模型把 treatment 效应放入独立分支，文章中在未重采样原始数据上表现更稳。
...
\mu_t(x)
=P(Y>0\mid x,t)
\cdot E[Y\mid Y>0,x,t]
$$

非随机日志升级 DR-Learner；数据量大、希望统一网络时可使用双头 TARNet。

#### D. 评估层

评估集必须是独立随机实验，或在评估集内重新完成 matching/IPW，不能在训练集上算。
...
4. 检查Top→Bottom是否递减；
5. 绘制累计Gain/Qini曲线并计算AUUC；
6. Bootstrap用户/Query簇置信区间；
7. 比较真实实验ATE与平均预测ATE；
8. 使用IPS/DR Policy Value评估最终展示/投放策略。

广告 Outcome 必须从平台整体视角定义，不能混淆 GMV 与平台营收口径：

```text
整体GMV = 广告商品GMV + 自然商品GMV
平台营收 = 广告CPC消耗 + 自然交易抽佣（按飞猪文章口径）
```

不能只看广告自身CTR/CVR/GMV，否则广告挤占自然结果造成的损失会被漏掉。

在线实验：

- 广告：平台总营收、整体GMV、广告主ROI、自然交易损失、CTR/CVR护栏；
- 悬浮球：增量搜索量、搜索DAU、入口点击、用户负反馈、长期活跃；
- 在相同流量/预算下比较旧策略与 Uplift 策略。
...
- treatment 是系统分配券，而不是领取/核销；
- 每组记录真实分流概率；
- outcome 观察窗口一致，例如7日有效订单；
- 同一用户实验期固定 treatment，设置券间 washout period；
- control 必须是真实不发券，不能因为编码为0就把“高星96折”当control。

##### 历史非随机发券

运营常按会员等级、近期意图和价格敏感度发券，必须：

1. 记录运营规则和资格条件；
2. 使用所有 treatment 前 confounders 训练多分类 propensity：

$$
e_t(x)=P(T=t\mid X=x)
$$

3. 采用 stabilized IPW、DR-Learner 或多 treatment matching；
4. 检查每个人群对各券档是否有 overlap；
5. 对极小 propensity clip/trim；
6. 新券档若无实验覆盖，不能依靠模型直接外推上线。
...
- `q_j`：核销成本/补贴金额；
- `g_ij`：订单GMV或贡献毛利；
- 它们主要进入策略价值函数，也可以作为 treatment 属性帮助泛化。
...
对同一用户遍历全部 treatment，得到价敏曲线：

```text
[p₀,p₁,...,p_K]
```

适合券档较多、需要一个模型统一维护，但要防止 treatment 被强用户特征忽略。
...
$$
\hat\tau_j(x)=\hat\mu_j(x)-\hat\mu_0(x)
$$

适合券档少、每组样本充分，用于校验 S-Learner 曲线。

##### 主模型：TARNet/Shared Multi-head
...
训练时只监督实际 treatment head；预测时：

$$
\tau_j(x)=\mu_j(x)-\mu_0(x)
$$

券档很多时使用 treatment embedding/conditioned head，避免 K 个完全独立大网络。

##### 非随机数据：DR

第一阶段估计 outcome 与 propensity，cross-fitting 构造：
...
$$
\tau_j(x)=E[Y_j^{DR}-Y_0^{DR}\mid X=x]
$$
...
- 不能各券分别取Top人群后直接合并；
- 对每个用户唯一选择券档；
- 使用 IPS/SNIPS/DR Policy Value；
- 评估券档分配比例、预算、增量订单、CPIO、iROI；
- Bootstrap置信区间。
...
### 11.4 第三块：有约束、成本控制和增量ROI的智能定价

这一层不是另一种 Uplift 模型，而是**把模型输出变成业务动作的策略优化层**。

#### A. 样本层

除上述 `X/T/Y/propensity` 外，还必须记录：
...
- 用户实际可选券集合；
- 当日预算、城市预算、渠道预算；
- 旧策略给每个动作的分配概率，供离线Policy Value使用。

必须区分两种成本：

```text
发放即产生成本：Cost_ij=q_j
核销才产生成本：Cost_ij=P_ij×q_j
```
...
Z_{ij}^{profit}
=
\delta P_{ij}\cdot ContributionMargin_{ij}
-P_{ij}q_j
$$

> `增量GMV/营销成本>1` 只代表GMV覆盖成本，不必然代表利润不亏。若毛利率低，必须使用增量贡献毛利或净利润口径。

#### C. 策略模型层

##### 第一版：半智能阈值

每档券人工寻找能覆盖成本的 uplift 阈值：
...
优点是稳健、易灰度；缺点是券间冲突、无法全局最优、阈值依赖人工。
...
$$
\sum_{i,j}Cost_{ij}x_{ij}\le Q
$$

整体ROI不亏约束：

$$
\sum_{i,j}\Delta Value_{ij}x_{ij}
\ge
\sum_{i,j}Cost_{ij}x_{ij}
$$
...
\arg\max_j
\left[
Z_{ij}-\lambda Cost_{ij}
\right]
$$

通过二分调整 `λ`，直到预算接近 `Q`。`λ` 是预算影子价格：
...
5元券ROI最高，但20元券创造的绝对净价值更大。合理策略是：
...
```text
S-Learner输出favor_j≈P_ij
→ KMeans用户桶
→ MIP按平均补贴率约束最大化订单/GMV
```
...
4. 将MIP目标改为增量净价值；
5. 增加整体ROI≥1约束；
6. 使用真实策略 propensity 做离线Policy Value；
7. 删除人工构造 `favor_0` 的演示逻辑。

#### D. 评估层

离线不能只比较预测ROI，需要评估完整策略：

1. 响应模型：每券 AUUC/Qini、分桶校准；
2. 策略价值：IPS/SNIPS/DR Policy Value；
3. 成本回放：预算消耗、券档比例、核销成本；
4. 业务价值：增量GMV、增量贡献毛利、净利润；
5. 效率：

$$
iROI_{GMV}=\frac{\Delta GMV}{MarketingCost}
$$

$$
iROI_{profit}
=
\frac{\Delta ContributionMargin}{MarketingCost}
$$

6. 风险：退款、取消、薅羊毛、长期补贴依赖；
7. 预算敏感性：绘制不同预算/λ下的GMV—利润Pareto曲线。
