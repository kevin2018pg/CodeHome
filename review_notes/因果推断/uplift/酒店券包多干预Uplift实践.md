...
> 本文用六个场景统一说明 Uplift 的工程落地：酒店券包与红包、旅行多品类券位 Top-2、搜索悬浮球、飞猪搜索广告、33 个补贴组合 ABR、Bidding 流量扶持。每个项目按**样本、特征、模型、评估、决策**展开；第十二章为 FAQ 与上线排查。
...
阈值版 / λ / LP 见上节。落地时一并挂上：日与城市预算、券库存、每用户唯一动作（或 Top-2）、频控准入、最低 ROI、补贴率上限、风控体验、探索流量。  
再次强调：不要选 ROI 最大；ROI 是约束，目标是总净价值。
...
| 问题 | 核心回答 |
| ---- | -------- |
| 没有随机实验能否做 | 混杂完整且有 overlap 时用 Matching/IPW/DR；国内主流是探索桶+去偏，最终仍要随机验收 |
| 场景偏置算不算随机 | 不算。特征与 propensity 吃场景/意图；分场景评估 |
| 核销能当 treatment 吗 | 不能（post-treatment）；主分析用 ITT（见 3.1） |
| 随机实验还要估 propensity 吗 | 等概率不用估；不等比例用日志概率；Policy Value 仍需要 e |
| 线上要跑 propensity 吗 | 一般不；只服务离线去偏/评估（见 3.3） |
| IPW 权重爆炸怎么办 | clip/trim、SNIPS、改 DR/Matching、加探索覆盖 |
| T / S / TARNet / DR | 少 T→T；共享→Heads；非随机→DR（见第九章） |
| 多 head 训练/预测 | 训练只监督 factual；预测跑全部 head 再做差（见 4.4） |
| 预算约束在模型里吗 | 多数否；决策层用阈值 / V−λΔC / LP（见第十一章） |
| 为何不选最大 uplift / ROI | 要净价值；ROI/补贴率是约束不是目标 |
| ATT vs CATE | ATT=已处理者平均；发券个性化用 CATE（见 1.2） |
| Hurdle / ZILN / 分档还原 | Hurdle 两段；ZILN 零+lognormal；分档用代表值还原期望（见 5.4、9.6） |
| PSM vs IPW | 匹配对齐 vs 1/e 加权（见 3.6） |
| 9 券选 2 | 品类内共享+组合决策；勿扁平36类、勿三业务各取 top（见四续） |
| 多 T 只看 Qini？ | 不够；看完整策略的 DR Policy Value |
| AUC 高 uplift差 | AUC 看响应，uplift 看增量 |
| 旅行有随机仍有何坑 | 意图异质、搬量、触发成本、退款、位次、多触点、库存、疲劳（见 4.9） |
